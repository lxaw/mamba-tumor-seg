...
In this paper, we strive to maintain high accuracy in our image segmentation while also keeping low parameter counts. We primarily seek to use pyramidal pooling modules \cite{zhao2017pyramid} to improve accuracy, and use a variant of the Mamba \cite{gu2023mamba} architecture inspired by \cite{ultralightvmunet} to maintain small parameter sizes.
...
\end{align*}
\gr{Do these need to be numbered equations? -- I don't. Nvm, disregard unless you think so}
...
\gr{To address the suggestion from Tong for "an overview of the whole pipeline" we should probably explain how PP is incorporated. Either by describing the pipeline or making a graphic for the pipeline.}
...
\gr{$These should all be UL-VM-UNet, or ULVM_UNet if you are against the extra dash. UltraLight VisionMamba-UNet This in each subsection title and in the paragraphs.$}
...
\gr{Pyramidal pooling was used in each of the PVM blocks in the encoder and decoder networks. If we want to do similar to PP-UMamba we can do it just in the bottleneck. Let me know if we want this can I can do it fast.}
...
\section{Results}
We now compare our models against ResNet18, ResNet50, standard U-Mamba with bottleneck \gr{UMamba bottleneck rather than "with"}, UNet, and UltraLight VM-UNet. We train for 100 epochs on all models, and use dice-loss with Adam optimizer. We train on roughly 3000 images, and validate on roughly 400. we then test on roughly 400. We show the stats of our data in Figure \ref{fig:datastats}.
The results are shown in Figures \ref{fig:bars},\ref{fig:balls},\ref{fig:segs}.

\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{imgs/bars.png}
    \caption{\gr{Thoughts on this vs the two bar graph?}Each model tested as a bar chart. The upper half is the Dice Scores, and the bottom half is the parameter count, where M means "million".}
    \label{fig:bars}
\end{figure}
...
\begin{figure}[!t]
    ...
    \label{fig:balls}
\end{figure}
\gr{do we need a table of the params and scores? May be helpful.. Maybe stick one in the appendix. }
...
\begin{figure}[!t]
    ...
    \label{fig:segs}
\end{figure}
\gr{let's include more segmentation results. Maybe in appendix.}




\gr{Would appendix really need to be in the page limit? Appendix is to include figures that wouldn't fit in paper?}
...
Our experiments and comparisons with state-of-the-art (SOTA) models demonstrate the potential of the Mamba-based approach in both model size and segmentation accuracy. The smallest model, ULVM\_Net\_v2, achieved a compelling dice score of 0.8243 with only 0.042M parameters, outperforming the larger ResNet50 model (42.9M params) \gr{how does this outperform? Dice score is lower.}with a dice score of 0.8672, while also outperforming the original UltraLight VM-UNet in terms of both parameter count and dice score.

While the traditional U-Net model achieved the highest dice score of 0.8929, our PP-UMamba model closely followed with a dice score of 0.8867 \gr{note the raise of 300k params in pp?}. Notably, our Mamba-based PP-UMambaBot \gr{name the section in OUR MODELS PP-UMambaBot or change this one. Consistency.} and UMambaBot models outperformed the Segformer transformer model, indicating the potential of Mamba-based approaches to surpass transformer-based models for this task.
