...
\begin{abstract}
    ...
    Our methodology involves implementing and fine-tuning a Mamba-based architecture on a dataset of annotated brain MRI scans. We will evaluate the model's performance using standard segmentation metrics and compare the results with state-of-the-art (SOTA) segmentation approaches. We open-source the code here: \href{https://github.com/lxaw/mamba-tumor-seg}{https://github.com/lxaw/mamba-tumor-seg}.
\end{abstract}
...
Our goal was to 1) maintain dice scores comparable to SOTA methods such as those listed above, while 2) being smaller than those above.
...
Model results are shown in Table \ref{tab:model_comparison} and Figures \ref{fig:balls},\ref{fig:segs}, with additional segmentation results in Appendix \ref{supp_figs}. Additional training information can be found in Appendix \ref{training}.
...
\subsection{Training Pipeline} \label{training}
...
UMambaBot-PP, UMambaBot, ResNet18, ResNet50, and UNet were trained for 100 epochs using DiceLoss as the loss function and Adam as the optimizer with a learning rate of 3e-4.

UL-VM-UNet and UL-VM-UNet-v1 through v6 were trained for 100 epochs using BCEDiceLoss as the loss function and AdamW as the optimizer with a learning rate of 3e-4.  
